...
int main(int argc, char **argv) {
  std::string bag, output = ".", prior_file;
  std::string wheel_topic = "/agt/chassis/odometry";
  std::string lio_topic = "/agt/mapping/odometry";
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "--bag") bag = value(a);
    ...
    else if (a == "--prior") prior_file = value(a);
      ...
      std::cout << "Usage: calibrate_bag --bag BAG_URI [--prior yaml] [--output DIR]\n";
      return 0;
    }
  }
  ...
  try {
    ...
    if (!prior_file.empty()) {
      prior = load_extrinsic_prior(prior_file);
      std::cout << "Using extrinsic prior: " << prior_file << "\n";
    }
    ...
    auto report = optimize_extrinsic(pairs, prior);

    write_result(report,
                 output + "/results/mid360_to_base.yaml",
                 output + "/results/optimization_report.md");

    std::cout << "wheel poses: " << wheel.size()
              << ", lio poses: " << lio.size()
              << ", motion pairs: " << pairs.size() << "\n";
    std::cout << "final RMSE: " << report.final_rmse << "\n";
